...
def _rdkit_ff_relax(molecule: Molecule, steps: int) -> Tuple[List[List[float]], str]:
    """Relax ``molecule`` with a bonded force field. Returns ``(coords, ff_name)``.
...
def preoptimize(
    ...
    try:
        coords, ff_name = _rdkit_ff_relax(molecule, steps)
