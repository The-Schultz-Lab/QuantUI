...
6. **Python 3.9 support is claimed but untested** — `requires-python = ">=3.9"` and a 3.9 classifier, but CI runs only 3.10/3.11. Care was clearly taken (e.g. the `StrEnum` shim in `viz_backend_router.py`), but nothing verifies it. — ✅ Fixed (added 3.9 to the Linux CI matrix; actual pass/fail on 3.9 will only be known once this branch's workflow runs against `main`)
